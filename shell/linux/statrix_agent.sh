...
if [ -f "$ScriptPath"/statrix.cfg ]
then
	. "$ScriptPath"/statrix.cfg
else
	exit 1
fi
...
then
	...
	# Clear statrix_cron.log every hour
	if [ -f "$ScriptPath"/statrix_cron.log ]
	then
		rm -f "$ScriptPath"/statrix_cron.log
		if [ "$DEBUG" -eq 1 ]; then echo -e "$ScriptStartTime-$(date +%T]) Cleared statrix_cron.log" >> "$ScriptPath"/debug.log; fi
	fi
fi

if [ "$DEBUG" -eq 1 ]; then echo -e "$ScriptStartTime-$(date +%T]) Starting Statrix Agent v$Version" >> "$ScriptPath"/debug.log; fi

# Kill any lingering agent processes
HTProcesses=$(pgrep -f statrix_agent.sh | wc -l)
if [ -z "$HTProcesses" ]
then
	HTProcesses=0
fi
if [ "$DEBUG" -eq 1 ]; then echo -e "$ScriptStartTime-$(date +%T]) Found $HTProcesses agent processes\n$(ps aux | grep 'statrix_agent.sh')" >> "$ScriptPath"/debug.log; fi
...
then
	...
	pgrep -f statrix_agent.sh | xargs -r kill -9
fi
if [ "$HTProcesses" -ge 10 ]
then
	for PID in $(pgrep -f statrix_agent.sh)
	do
...
then
	...
	do
		...
		bash "$ScriptPath"/statrix_agent.sh ping "${OutgoingPing[0]}" "${OutgoingPing[1]}" &
	done
fi
...
# Save data to file
echo "j=$jsoncomp" > "$ScriptPath"/statrix_agent.log
...
then
	...
	wget -v --debug --retry-connrefused --waitretry=1 -t 3 -T 15 -O- --post-file="$ScriptPath/statrix_agent.log" $SecuredConnection "$STATRIX_ENDPOINT/v2/" &>> "$ScriptPath"/debug.log
	echo -e "$ScriptStartTime-$(date +%T]) Data posted" >> "$ScriptPath"/debug.log
else
	# Post data
	wget --retry-connrefused --waitretry=1 -t 3 -T 15 -qO- --post-file="$ScriptPath/statrix_agent.log" $SecuredConnection "$STATRIX_ENDPOINT/v2/" &> /dev/null
fi
